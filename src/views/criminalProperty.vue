<template>
   ...
   <div class="absolute top-28 left-7 bg-white rounded-md w-5/6 px-5 shadow-sm pb-10 lg:top-32 lg:left-40 lg:px-20 lg:w-3/4">
       ...
       <div class="justify-center flex">
           ...
           <div>
            <h1 class="text-[#0d5364] text-center pt-4  font-bold text-xl lg:text-3xl">ኮምቦልቻ ማረሚያ ቤት</h1>
             
           </div>
           
       </div>
       <div>
        <h1 class="text-[#0d5364] mt-1 font-bold text-xl lg:text-2xl lg:-mt-1 lg:w-1/3 lg:mx-auto">የታራሚዉን ንብረት ይመዝግቡ </h1>
       </div>
       
       <div class=" lg:w-1/2 mt-16 lg:mx-auto">
        <div>
            <label for="" class="text-[#0d5364]">የታራሚዉ ስም </label>
            <select name="" id="" v-model="model.criminalProperties.criminal_id" class="bg-gray-100 rounded-sm mt-2 w-12/13 h-10 pl-3 border border-gray-200 focus:outline-[#1e88ab] lg:w-100">
           ...
           <div class="mt-10">
            <label for="" class="text-[#0d5364]">የንብረት አይነት</label>
            <select name="" id="" v-model="model.criminalProperties.type_id" class="bg-gray-100 rounded-sm mt-2 w-12/13 h-10 pl-3 border border-gray-200 focus:outline-[#1e88ab] lg:w-100">
                <option value="" disabled>የንብረት አይነት ይምረጡ </option>
                <option v-for="type in types" :key="type.id" :value="type.id">{{ type.type }}</option>
            </select>
           </div>
      <div class="mt-10">
          <p class="text-[#0d5364]">ብዛት</p>
          <input type="text" name="" id="" class="bg-gray-100 rounded-sm mt-2 w-12/13 h-10 pl-3 border border-gray-200 focus:outline-[#1e88ab] lg:w-100" placeholder="ብዛት" v-model="model.criminalProperties.amount">
      </div>
       
      <div>
          <button class="bg-[#0d5364] text-white px-7 rounded-sm py-2 ml-1 mt-6" @click="registerCriminalProperty">ይመዝግቡ </button>
      </div>
  </div>
</div>
